...
despot_result_t flac_picture_parse(despot_ctx_t* ctx, io_t* io) {
  // Relevant: https://www.rfc-editor.org/rfc/rfc9639.html
  
  despot_picture_t picture = { 0 };
  
  MUST(io_read_u32_be(io, &picture.type));
  if (picture.type > DESPOT_PICTURE_PUBLISHER_OR_STUDIO_LOGO) {
    picture.type = DESPOT_PICTURE_OTHER;
  }
  
  uint32_t media_type_length;
  MUST(io_read_u32_be(io, &media_type_length));
  
  picture.media_type = malloc(media_type_length+1);
  TRY(io_read(io, (char*)picture.media_type, media_type_length)) CATCH (
    cleanup(&picture);
  );
  ((char*)picture.media_type)[media_type_length] = '\0';
  
  uint32_t description_length;
  TRY(io_read_u32_be(io, &description_length)) CATCH (
    cleanup(&picture);
  );
  
  picture.description = malloc(description_length+1);
  TRY(io_read(io, (char*)picture.description, description_length)) CATCH (
    cleanup(&picture);
  );
  ((char*)picture.description)[description_length] = '\0';
  
  TRY(io_read_u32_be(io, &picture.width)) CATCH (
    cleanup(&picture);
  );
  
  TRY(io_read_u32_be(io, &picture.height)) CATCH (
    cleanup(&picture);
  );
  
  TRY(io_skip(io, 8)) CATCH (
    cleanup(&picture);
  );
  
  uint32_t size;
  TRY(io_read_u32_be(io, &size)) CATCH (
    cleanup(&picture);
  );
  picture.size = size;
  
  picture.source_offset = io_tell(io);
